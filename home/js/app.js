...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('promptForm').addEventListener('submit', async function (event) {
    ...
    const useFixedSeed = document.getElementById('fixed-seed-checkbox').checked;
    const fixedSeedValue = parseInt(document.getElementById('fixed-seed-value').value);
    ...
    try {
      ...
      if (useFixedSeed && !isNaN(fixedSeedValue)) {
        seedUsed = fixedSeedValue;
      } else {
        seedUsed = Math.floor(Math.random() * 18446744073709551614) + 1;
      }

      promptWorkflow["3"]["inputs"]["seed"] = seedUsed;
      promptWorkflow["6"]["inputs"]["text"] = promptText;

      // Mostrar la semilla usada
      document.getElementById('seed-value').textContent = seedUsed;
  ...
  // Activar/desactivar input de semilla
  document.getElementById('fixed-seed-checkbox').addEventListener('change', () => {
    const input = document.getElementById('fixed-seed-value');
    input.disabled = !document.getElementById('fixed-seed-checkbox').checked;
  });
});
